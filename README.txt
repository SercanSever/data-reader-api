...
#### Bu katmanda web.config dosyasında konfigürasyon olarak tutulan dosyaların endpoint ile değiştirilmesi için gerekli 
kodlama yapılmıştır.
#### Bu katmanda gerekli olan Custom Exceptionlar oluşturulmuştur.

## Apı Katmanı

#### Api katmanında iki controller oluşturulmuştur. Bir controller dosya içeriğinin gösterilmesiyle ilgilenirken
diğer controller kullanılacak olan verinin değiştirilmesini endpoint ile sağlamaktadır.
...
## Factory Design Pattern ile veri kümeleri interface ile implement edilmiştir. Projede veri kümeleri
birbirine benzediği için her seferinde new operatörü ile bir nesne türetmektense ya da birbirlerinden tamamen
bağımsız gibi kodlamaktansa bu desenle tek bir interface ile birleştirilmiştir. Uzantısı farklı olan bir dosya 
projeye eklendiği takdirde projede hiç bir değişikliğe gerek duymadan sadece yeni dosyanın okunma işlemi yapılmalıdır.
...
## OData ile dosya içeriğinin filtrelenmesi tek bir metotta bir Attribute ile sağlanmıştır. Gerekli olan
filtrelemeler;
